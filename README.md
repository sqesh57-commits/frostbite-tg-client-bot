...
### Защита создания профилей

| Переменная | Описание | По умолчанию |
|-----------|----------|-------------|
| `BOT_REQUIRE_ADMIN_FOR_PROFILE_CREATE` | Включает дополнительные бизнес-проверки перед self-service созданием профиля: активная подписка/trial, отсутствие существующего профиля, blacklist и rate limit. Обычные пользователи с активным доступом не блокируются. | `false` |
| `BOT_BLOCKED_PROFILE_CREATE_IDS` | Telegram ID пользователей, которым запрещено создавать новый профиль, через запятую. | |
| `BOT_PROFILE_CREATE_RATE_LIMIT_SECONDS` | Минимальный интервал между попытками создания профиля для одного Telegram ID. | `60` |
| `BOT_MAX_PROFILES_PER_USER` | Максимальное число профилей на пользователя; текущее хранилище поддерживает один профиль, значение `0` временно запрещает создание. | `1` |
...
Для тестирования можно удалить `users.db` прямо внутри контейнера. Скрипт показывает число пользователей до/после сброса, удаляет сам файл SQLite и sidecar-файлы (`-wal`, `-shm`, `-journal`). Если БД находится в bind mount (`./data:/app/data`), скрипт сначала останавливает контейнер и удаляет файл на хосте, затем запускает контейнер заново, чтобы бот создал пустую схему.
...
Если нужно полностью сбросить тестовый аккаунт, включая уже созданные в 3x-ui профили пользователей из текущей БД, добавьте `--purge-xui`. Это важно, потому что очистка `users.db` не удаляет клиентов из панели 3x-ui.

```bash
./reset_db.sh --purge-xui
```

> Используйте только в тестовом окружении: данные пользователей и найденные профили 3x-ui будут удалены без восстановления.
